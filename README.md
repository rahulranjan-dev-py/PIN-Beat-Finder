...
| **Local Beats (offline)** | Room-backed directory of localities → Beat / office (type + name) / account office / PIN. With the search box empty the tab lists offices; tapping one opens its villages grouped by beat. Debounced (250 ms) search combines substring, exact beat/PIN and **phonetic** matching (Double Metaphone with an Indian-transliteration pre-normaliser). A Filters sheet narrows by State, District, Office type, Office, Beat and PIN with cascading choices. |
| **CRUD** | Add/edit via a modal bottom sheet: office type dropdown (GPO/HO/IDC/SO/BO), Office Name suggestions from the built-in directory as you type, and a **Fetch** button that lists every office under the typed PIN (with how many local beats/villages each already has) so one tap fills type, name, account office, district and state. Ticking several offices in the picker enters them one after another ("Save & next"). Account Office suggests SO/HO/GPO offices from the directory. The By-beat view's **Office types** chip re-types every record of an office in one go and flags offices with mixed types; each beat card can share that beat or the whole office as `.xlsx` or as a printable A4 PDF. While typing a locality the form warns about existing records with the same or a similar-sounding name; "Find duplicate villages" scans the whole directory and lets you keep one of each pair. Delete with confirmation. Validation is shared with the importer. |
